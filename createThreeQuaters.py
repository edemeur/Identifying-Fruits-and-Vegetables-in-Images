...
# Creates the  Three Quater dataset
...
for i in range(3):
    if i == 0:
        temp_dir = "archive/combined_threequater/Train/"
    elif i == 1:
        temp_dir = "archive/combined_threequater/Valid/"
    else:
        temp_dir = "archive/combined_threequater/Test/"
    for name in names:
        dir = temp_dir + name
        os.mkdir(dir)
...
for idx, superset in enumerate(all_images):
    for set in superset:
        ...
        for image in set:
            for i in range(0, 4):
                new_image = np.full((100,100,3), 255)
                if i == 0:
                    for x in range(0, 50):
                        for y in range(0, 50):
                            new_image[x][y] = copy.deepcopy(image[x][y])
                if i == 1:
                    for x in range(0, 50):
